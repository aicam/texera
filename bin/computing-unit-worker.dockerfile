...
RUN PROTOC_VERSION=$(cat bin/protoc-version.txt) \
    && case "$(uname -m)" in \
         x86_64|amd64) PROTOC_PLATFORM=linux-x86_64 ;; \
         aarch64|arm64) PROTOC_PLATFORM=linux-aarch_64 ;; \
